...
function gerarResumoMensal(mes, ano) {
  ...
  // --- PRIMEIRO PASSO: Calcular Despesas e Receitas - Misto ---
  // CORRIGIDO: O cálculo do fluxo de caixa e das despesas detalhadas agora usa a Data de Vencimento para despesas e a Data da Transação para receitas.
  logToSheet("Iniciando PRIMEIRO PASSO: Calcular Despesas e Receitas - Misto.", "INFO");
  for (let i = 1; i < transacoes.length; i++) {
    const dataRaw = transacoes[i][0]; // Data da transação
    ...
    const dataVencimentoRaw = transacoes[i][10]; // Data de Vencimento
    ...
    if (tipo === "Receita") {
      ...
      if (!(categoriaNormalizada === "transferencias" && subcategoriaNormalizada === "entre contas")) {
          ...
          logToSheet(`[Fluxo de Caixa] Transacao ID: ${idTransacao} é Receita (excluindo transferencias). Total Receitas: ${totalReceitasMes}`, "DEBUG");
      } else {
          logToSheet(`[Fluxo de Caixa] Transacao ID: ${idTransacao} (${categoria} > ${subcategoria}) excluída do cálculo de Receitas Totais (Fluxo de Caixa).`, "DEBUG");
      }
    }
    // Se for DESPESA, filtra pela DATA DE VENCIMENTO
      ...
      if (
          ...
          // Também adiciona ao resumo de categorias para DESPESAS, pois o filtro de data é o mesmo agora
          ...
          logToSheet(`[Fluxo de Caixa] Transacao ID: ${idTransacao} (${categoria} > ${subcategoria}) excluída do cálculo de Despesas Totais (Fluxo de Caixa).`, "DEBUG");
      }
    }
  }
  logToSheet("PRIMEIRO PASSO concluido: Fluxo de Caixa e Despesas Detalhadas por Categoria calculados.", "INFO");

  // --- SEGUNDO PASSO: Calcular Gastos de Cartão de Crédito no Mes (Fatura Bruta) - USA DATA DE VENCIMENTO ---
  // Mantido, pois a lógica já estava correta (usando data de vencimento)
  logToSheet("Iniciando SEGUNDO PASSO: Calcular Gastos de Cartao de Credito no Mes (Fatura Bruta) - USA DATA DE VENCIMENTO.", "INFO");
  for (let i = 1; i < transacoes.length; i++) {
  ...
  logToSheet("SEGUNDO PASSO concluido: Gastos de Cartao de Credito no Mes calculados.", "INFO");

  // --- TERCEIRO PASSO: Processamento de Metas - USA DATA DE VENCIMENTO ---
  // A lógica de metas também deve usar a data de vencimento para ser consistente com o planejamento de faturas
  logToSheet("Iniciando TERCEIRO PASSO: Calcular Gastos de Metas - USA DATA DE VENCIMENTO.", "INFO");
  for (let i = 1; i < transacoes.length; i++) {
    const dataVencimentoRaw = transacoes[i][10]; // Data de Vencimento
    const dataVencimento = parseData(dataVencimentoRaw);
    const tipo = transacoes[i][4];
    const categoriaTransacao = transacoes[i][2];
    const subcategoriaTransacao = transacoes[i][3];
    const valor = parseBrazilianFloat(String(transacoes[i][5]));

    if (dataVencimento && dataVencimento.getMonth() === mesIndex && dataVencimento.getFullYear() === ano && tipo === "Despesa") {
      const chaveCategoriaMeta = normalizarTexto(categoriaTransacao);
      const chaveSubcategoriaMeta = normalizarTexto(`${categoriaTransacao} ${subcategoriaTransacao}`);
      if (metasPorCategoria[chaveCategoriaMeta] && metasPorCategoria[chaveCategoriaMeta].subcategories[chaveSubcategoriaMeta]) {
        metasPorCategoria[chaveCategoriaMeta].subcategories[chaveSubcategoriaMeta].gasto += valor;
        metasPorCategoria[chaveCategoriaMeta].totalGasto += valor;
      }
  ...
  logToSheet("TERCEIRO PASSO concluido: Gastos de Metas calculados.", "INFO");
  ...
  try {
    ...
    if (categoriasOrdenadas.length === 0) {
        ...
        categoriasOrdenadas.forEach(categoria => {
          try {
            ...
            // CORRIGIDO: Cálculo da porcentagem da meta
            const metaExistente = metaInfo.totalMeta || 0;
            const gastoMeta = metaInfo.totalGasto || 0;
            if (metaExistente > 0) {
              const percMeta = (gastoMeta / metaExistente) * 100;
              let emojiMeta = "";
              if (percMeta >= 100) emojiMeta = "⛔";
              else if (percMeta >= 80) emojiMeta = "⚠️";
              else emojiMeta = "✅";
              mensagemResumo += ` ${emojiMeta} (${percMeta.toFixed(0)}% da meta de R$ ${metaExistente.toFixed(2).replace('.', ',')})`;
            }
            mensagemResumo += `\n`;
            ...
            subcategoriasOrdenadas.forEach(sub => {
              try {
                ...
                let subLine = `  • ${escapeMarkdown(capitalize(sub))}: R$ ${gastoSub.toFixed(2).padStart(7).replace('.', ',')}`;
                if (subMetaInfo && subMetaInfo.meta > 0) {
                  let subEmoji = "";
                  // CORRIGIDO: Cálculo da porcentagem da sub-meta
    ...
    // CORRIGIDO: Declaração da variável 'cartoesOrdenados'
    const cartoesOrdenadas = Object.keys(resumoCartoes).sort((a, b) => resumoCartoes[b].faturaBrutaMes - resumoCartoes[a].faturaBrutaMes);
    if (cartoesOrdenadas.length === 0) {
      ...
      cartoesOrdenadas.forEach(cartaoNormalizadoKey => { 
        try { 
...
function enviarSaldo(chatId, usuario) {
  ...
  try {
    ...
    // CORRIGIDO: Agora a lógica é a mesma que o dashboard usa para billsToPay
    const billsToPay = [];
    if (contasAPagarData.length > 0) {
        ...
        for (let i = 1; i < contasAPagarData.length; i++) {
            ...
            if (dataVencimento && dataVencimento.getMonth() === currentMonth && dataVencimento.getFullYear() === currentYear) {
                // Aqui, a lógica se torna mais inteligente e não depende de texto específico
                const descricao = (row[colDescricao] || "").toString().trim();
                const valor = parseBrazilianFloat(String(row[colValor] || '0'));
                const status = (row[colStatus] || "").toString().trim();
                
                // Filtra apenas contas que são pagamentos de cartão ou de despesas fixas
                if (normalizarTexto(descricao).includes("fatura") || normalizarTexto(row[contasAPagarHeaders.indexOf('Categoria')]).includes('despesas fixas')) {
                    billsToPay.push({
                        descricao: descricao,
                        valor: valor,
                        status: status
    ...
    billsToPay.sort((a, b) => a.descricao.localeCompare(b.descricao)).forEach(fatura => {
